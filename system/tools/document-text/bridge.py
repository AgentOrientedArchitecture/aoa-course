"""AOA bridge for the document-text MCP server.

The bridge keeps the course system's registered capability shape stable while
the actual document extraction is provided by an MCP tool.
"""
...
logger = logging.getLogger("document-text-bridge")
...
async def invoke(request: Request) -> JSONResponse:
    ...
    try:
        mcp_result = await mcp.request(
            "tools/call", {"name": "extract_text", "arguments": {"path": path}}
        )
        ...
        return JSONResponse(
            {
                ...
                "outputs": {},
                "signals": {
                    "path_within_root": "outside allowed root" not in error,
    ...
    text = _first_text(mcp_result)
    return JSONResponse(
        {
            "trace_id": trace_id,
            "outputs": {
                "text": text,
                "media_type": mcp_result.get("media_type", "application/octet-stream"),
            },
            "signals": {
